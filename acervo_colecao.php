...
var i = valor;
document.location=('acervo_colecao.php?page='+ i+ '&titulo=<? echo $_REQUEST[titulo] ?>&vinculo=<? echo $_REQUEST[vinculo] ?>&funcao=<? echo $_REQUEST[funcao] ?>&cor=<? echo $_REQUEST[cor] ?>');
}
...
?>
  ...
  <tr>
    ...
    <?
	  ...
	  $sql="SELECT count(*) T  FROM colecao a, obra b
            WHERE (a.colecao = b.colecao)group by b.colecao ORDER BY T DESC";
	  $db->query($sql);
	  $numlinhas=$db->contalinhas();
	 
	  $sql2="SELECT count(a.nome) FROM colecao a, obra b WHERE a.colecao=b.colecao ";
	  $db->query($sql2);
	  $tot_geral=$db->dados();
	 
	  $sql3="SELECT a.nome,a.colecao as numero, count( * ) T,b.obra FROM colecao a, obra b
      WHERE (a.colecao = b.colecao) GROUP BY b.colecao ORDER BY T DESC   LIMIT $registroinicial,$pagesize";
	 ...
	 ?>
      <table width="100%" height="20"  border="0" cellpadding="0" cellspacing="0">
        ...
        <tr bgcolor="#96ADBE">
          <td width="44%" height="24" bgcolor="#96ADBE" class="texto_bold">&nbsp;&nbsp;Cole&ccedil;&otilde;es</td>
          <td width="12%" bgcolor="#96ADBE" class="texto_bold"> <div align="center">N&uacute;mero </div></td>
          <td width="15%" bgcolor="#96ADBE" class="texto_bold"><div align="center">N&ordm; de obras </div></td>
          <td width="15%" bgcolor="#96ADBE" class="texto_bold"><div align="center">              %</div></td>
          <td width="14%" bgcolor="#96ADBE" class="texto_bold"><div align="center">&nbsp;</div></td>
        </tr>
        <tr>
	  ...
	  ?>
        <tr class="texto" id="cor_fundo<? echo $row[obra] ?>">
          <td align="justify" width='44%'><b>&nbsp;<? echo $row[nome] ?></b> 
			</td>
          <td align="center" width='12%'><? echo $row[numero] ?></td>
          ...
          <td align="center" width='14%'><? if ($_REQUEST[pagesize] < 999) echo "<a href=\"acervo_colecao1.php?colecao=".$row[numero]."\">
	 ...
	 onMouseOut='document.getElementById(\"cor_fundo".$row[obra]."\").style.backgroundColor=\"\";'>"; }?>
        ...
        <tr class="texto">
          <td colspan="8" class="noprint"><? if ($_REQUEST[pagesize] < 999) echo "<a target='_blank' href=\"acervo_colecao.php?page=1&pagesize=999999\"><img src='imgs/icons/ic_salvar_impressao.gif'  border='0'  alt='Versão para impressão'></a>" ?></td>
        </tr>
        <tr>
...
$a="<a href=\"acervo_colecao.php?page=".$first."\"><img src='imgs/icons/btn_inicio.gif'  border='0'  alt='Registro Inicial'></a>";

$b="<a href=\"acervo_colecao.php?page=".$menos."\"><img src='imgs/icons/btn_anterior.gif'  border='0' alt='Registro Anterior'></a>";

$c="<a href=\"acervo_colecao.php?page=".$mais."\"><img src='imgs/icons/btn_proximo.gif'  border='0' alt='Proximo Registro'></a>";

$d="<a href=\"acervo_colecao.php?page=".$last."\"><img src='imgs/icons/btn_ultimo.gif'  border='0' alt='Ultimo Registro'></a>";
...
$g= " ".$tot_geral[0]." obras em ".$numlinhas." coleções ".$txtpagina.$a."&nbsp".$b."&nbsp".$c."&nbsp".$d."";
echo"&nbsp";
