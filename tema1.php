...
function valida()
{
 with(document.form1){
    if(nome.value==''){
	  alert('Preencha com o nome do tema');
	  nome.focus();
	  return false;}
 }
}
...
$db=new conexao();
$db->conecta();
$val=base64_decode($_REQUEST['tema']);
$op=$_REQUEST['op'];
echo $_SESSION['lnk'];
?>
</div></th>
...
{ 
 if($op=='update')
 {
   $sql="SELECT nome from tema as a where a.tema='$val'";
   $db->query($sql);
   $res=$db->dados();
  }
 if($op=='del')
 {
     $sql="DELETE from tema where tema='$val'";
	 $db->query($sql);
...
<table width="100%"  border="0" cellpadding="0" cellspacing="4" >
        <tr>
          <td colspan="2" class="texto_bold"><div align="left"><br><br><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;T&iacute;tulo  do
              tema: &nbsp;              
        ...
        <tr>
          <td>
            <div align="left"><? echo "<a href=\"tema.php\"><img src='imgs/icons/btn_voltar.gif' width='20' height='20' border='0' alt='Voltar' >"?></div></td>
          <td>&nbsp;</td>
        </tr>
      </table>
      <br>
...
if($_REQUEST['enviar']<>'')
{
  if($_REQUEST[op]=='update')
   {
     $sql="UPDATE tema set nome='$_REQUEST[nome]' where tema='$_REQUEST[val]'";
	 $db->query($sql);
  ...
  {
     $sql= "INSERT INTO tema(nome) values('".trim($_REQUEST[nome])."')";
	 $db->query($sql);
