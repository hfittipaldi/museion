...
if (($modelo1==1) and ($modelo2==0)) fwrite($fh,"<td>AUTOR</td><td>NASCIMENTO</td><td>TITULO</td><td>TECNICA/MEDIDAS</td><td>ASSINATURA</td><td>AQUISIÇÃO</td><td>IMG</td>");
if (($modelo1==1) and ($modelo2==1)) fwrite($fh,"<td>AUTOR</td><td>NASCIMENTO</td><td>TITULO</td><td>TECNICA/MEDIDAS</td><td>ASSINATURA</td><td>AQUISIÇÃO</td><td>TITLE</td><td>TECH MEASURE</td><td>SIGNATURE</td><td>ACQUISITION</td><td>IMG</td>");
if ($modelo1==1) fwrite($fh,"</tr>");
      ...
      $count= 0;
	   while($row=$db->dados()) { 
		...
		$lista_autoria .= $autor['nomeetiqueta'];
                 if ($mort<>'') {$nascimentotxt.=$atribuido.$nasc." - ".$mort;}
                 else{$nascimentotxt.=$atribuido.$nasc;}
...
if ($modelo1==1) fwrite($fh,"$lista_autoria</td>");
...
if ($modelo1==1) fwrite($fh,"<td>");
					...
					if ($row['texto_etiq'] <> '') {
					   if ($modelo1==1) fwrite($fh,  $row['texto_etiq']);  $aquisicao_ing=$aquisicao_ing.", ".$row['texto_etiq'];
