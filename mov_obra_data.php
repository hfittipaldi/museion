...
{
 with(document.form1){
	if (data.value=='') { alert('Preencha a data de saída!'); data.focus(); return false; }
	if (!Validar_Campo_Data(data,false)) {
		alert('Preencha corretamente o campo "data de saída"!'); data.focus(); return false;
	}
 }
}
</script>  
...
if ($_REQUEST[op] == 'limpar') {
	$sql="UPDATE obra_movimentacao set data_saida='0000-00-00' where obra = '$obrid' AND movimentacao = '$movid'";
	$db->query($sql);
...
<?
   $sql="SELECT data_saida from obra_movimentacao as a where a.obra = '$obrid' AND a.movimentacao = '$movid'";
   $db->query($sql);
   $res=$db->dados();
	$data= explode("-", $res[0]);
	...
	if ($data=="00/00/0000" || $data=="//")
		$data= "";
?>
<table width="100%"  border="0" cellpadding="0" cellspacing="4" bgcolor="#f2f2f2">
        <tr>
          <td colspan="2" class="texto_bold"><div align="left">Data de saída da obra: &nbsp;              
        ...
        <tr>
          <td>
            <div align="left"><? echo "<a href=\"obra_lista.php?movid=".$movid."\"><img src='imgs/icons/btn_voltar.gif' width='20' height='20' border='0' alt='Voltar' >"?></div></td>
          <td>&nbsp;</td>
        </tr>
      </table>
      <br>
...
if ($_REQUEST['enviar'] <> '') {
	...
	if ($data == "--")
		$data= "00-00-0000";
     $sql="UPDATE obra_movimentacao set data_saida='$data' where obra = '$obrid' AND movimentacao = '$movid'";
	 $db->query($sql);
	 echo"<script>alert('Alteração efetuada com sucesso.')</script>";
	 echo"<script>location.href='obra_lista.php?movid=".$movid."'</script>";
}   
