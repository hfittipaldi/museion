...
document.location=('autor_pais.php?page='+i);
}}

function posiciona(valor) {
var i = valor;
document.location=('autor_pais.php?page='+ i+ '&titulo=<? echo $_REQUEST[titulo] ?>&vinculo=<? echo $_REQUEST[vinculo] ?>&funcao=<? echo $_REQUEST[funcao] ?>&cor=<? echo $_REQUEST[cor] ?>');
}
...
?>
  ...
  <tr>
    ...
    <?
	  ...
	  $sql="SELECT a.autor T  FROM autor a, pais b
            WHERE (a.pais_nasc = b.pais) group by b.pais ORDER BY T DESC";
	  $db->query($sql);
	  $numlinhas=$db->contalinhas();
	  $sql2="SELECT count( b.nome ) FROM autor a, pais b WHERE a.pais_nasc = b.pais";
	  $db->query($sql2);
	  $tot_geral=$db->dados();
	 
	  $sql3="SELECT  b.nome,b.pais,count( * ) T FROM autor a, pais b
            WHERE (a.pais_nasc = b.pais) GROUP BY b.pais ORDER BY T DESC  LIMIT $registroinicial,$pagesize";
	  $db->query($sql3); 
	 ...
	 ?>
      <table width="100%" height="20"  border="0" cellpadding="0" cellspacing="0">
        ...
        <tr bgcolor="#ddddd5">
          <td width="56%" height="24" bgcolor="#ddddd5" class="texto_bold">&nbsp;País</td>
	  ...
	  {
	  ?>
        <tr class="texto" id="cor_fundo<? echo $row[pais] ?>">
          ...
          <td align="center" width='14%'><? if ($_REQUEST[pagesize] < 999) echo "<a href=\"autor_pais1.php?pais=".$row['pais']."\">
	 <img src='imgs/icons/relat.gif' width='20' height='20' border='0' alt='Informações' 
	 onMouseOver='document.getElementById(\"cor_fundo".$row[pais]."\").style.backgroundColor=\"#ddddd5\";' 
	 onMouseOut='document.getElementById(\"cor_fundo".$row[pais]."\").style.backgroundColor=\"\";'>"; }?>
        ...
        <tr class="texto">
          <td colspan="7" class="noprint"><? if ($_REQUEST[pagesize] < 999) echo "<a target='_blank' href=\"autor_pais.php?page=1&pagesize=999999\"><img src='imgs/icons/ic_salvar_impressao.gif'  border='0'  alt='Versão para impressão'></a>" ?></td>
        </tr>
        <tr>
...
$a="<a href=\"autor_pais.php?page=".$first."\"><img src='imgs/icons/btn_inicio.gif'  border='0'  alt='Registro Inicial'></a>";

$b="<a href=\"autor_pais.php?page=".$menos."\"><img src='imgs/icons/btn_anterior.gif'  border='0' alt='Registro Anterior'></a>";

$c="<a href=\"autor_pais.php?page=".$mais."\"><img src='imgs/icons/btn_proximo.gif'  border='0' alt='Proximo Registro'></a>";

$d="<a href=\"autor_pais.php?page=".$last."\"><img src='imgs/icons/btn_ultimo.gif'  border='0' alt='Ultimo Registro'></a>";
...
$g= " ".$tot_geral[0]." autores de ".$numlinhas." Países ".$txtpagina.$a."&nbsp".$b."&nbsp".$c."&nbsp".$d."";
echo"&nbsp";
