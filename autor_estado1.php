...
document.location=('autor_estado1.php?page='+i+'&estado=<? echo $_REQUEST[estado]?>');
 }
}
...
?>
  ...
  <tr>
    <td colspan="2" valign="top"><form name="form1" method="post" action="">
      <?
 function ret_estado($valor) 
	  {
           global $db,$res;
	      $sql="SELECT nome from estado where estado='$valor'";
		  $db->query($sql);
	      $res=$db->dados();
		  $res=$res[0];
       }
ret_estado($_REQUEST[estado]);
	  ...
	  $sql="SELECT count(*) as total FROM autor a, estado b WHERE a.estado_nasc = b.estado AND b.estado = '$_REQUEST[estado]'";
	  $db->query($sql);
	  $numlinhas=$db->dados();
      $numlinhas=$numlinhas[0];
	 
	  /////////////////////
	    $sql2="SELECT a.* FROM autor a, estado b WHERE a.estado_nasc = b.estado AND b.estado='$_REQUEST[estado]'
	 ...
	 onMouseOut='document.getElementById(\"cor_fundo".$row[autor]."\").style.backgroundColor=\"\";'>";} ?>
        ...
        <tr class="texto">
          <td colspan="4" class="noprint"><? if ($_REQUEST[pagesize] < 999) echo "<a target='_blank' href=\"autor_estado1.php?pagesize=999999&page=1&estado=".$_REQUEST[estado]."\"><img src='imgs/icons/ic_salvar_impressao.gif'  border='0'  alt='Versão para impressão'></a>" ?></td>
        </tr>
        <tr>
...
$a="<a href=\"autor_estado1.php?page=".$first."&estado=".$_REQUEST[estado]."\"><img src='imgs/icons/btn_inicio.gif'  border='0'  alt='Registro Inicial' ></a>";

$b="<a href=\"autor_estado1.php?page=".$menos."&estado=".$_REQUEST[estado]."\"><img src='imgs/icons/btn_anterior.gif'  border='0' alt='Registro Anterior' ></a>";

$c="<a href=\"autor_estado1.php?page=".$mais."&estado=".$_REQUEST[estado]."\"><img src='imgs/icons/btn_proximo.gif'  border='0' alt='Proximo Registro' ></a> ";

$d="<a href=\"autor_estado1.php?page=".$last."&estado=".$_REQUEST[estado]."\"><img src='imgs/icons/btn_ultimo.gif'  border='0' alt='Ultimo Registro' ></a>";
$combo="";
