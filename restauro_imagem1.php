...
function obtem_tipo() {
 global $db;
 // 1=papel | 2=pintura | 3=Obra 3D | 4=moldura\\
...
function remove_foto()
{
 ...
 elseif ($tipo == 3)
	 $arq_rem= diretorio_fisico() . "Obra 3D\\$arq";
 elseif ($tipo == 4)
	 $arq_rem= diretorio_fisico() . "moldura\\$arq";
 unlink($arq_rem);
}
  ...
  if($_REQUEST[op]=='update')
   {
	...
	if ($ext <> 'arquivo ja carregado') {
		 ...
		 elseif ($tipo == 3)
			 $dir= diretorio_fisico() . "Obra 3D\\";
		 elseif ($tipo == 4)
			 $dir= diretorio_fisico() . "moldura\\";
		set_time_limit(0);
