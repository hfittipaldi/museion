...
var i = qual.value;
document.location=('logsistema.php?page='+ i+ '&dt_ini=<? echo $_REQUEST[dt_ini]; ?>&dt_fim=<? echo $_REQUEST[dt_fim]; ?>&autor=<? echo trim($_REQUEST[autor]); ?>&obra=<? echo $_REQUEST[obra]; ?><!PRD10!>');
}}
...
////////////
		if ($_REQUEST['op'] == 'del') {
		  ...
		  exit();
                ...
                // Consulta autor PBL $whereAutorCount
		

		if ($_REQUEST[autor]<>'') {
			$whereAutor="a.autor=".$_REQUEST[autor]." and ";
                        $whereAutorCount="autor=".$_REQUEST[autor]." and ";
                echo " .$WhereAutor.";
		...
		if ($_REQUEST[obra]<>'') {
			$WhereAutor="a.obra=".$_REQUEST[obra]." and ";
			$whereAutorCount="obra=".$_REQUEST[obra]." and ";
		} else {
			$whereAutor="";
			$WhereAutorCount="";
		}
		  ...
		  $sql="SELECT count(*) as total from log_atualizacao where ".$whereAutorCount." data between DATE_SUB(CURDATE(),INTERVAL 90 DAY) and now()";
		  $sql2="SELECT a.*, b.nome FROM log_atualizacao AS a LEFT JOIN usuario AS b on (a.usuario = b.usuario) 
			WHERE ".$whereAutor." a.data between DATE_SUB(CURDATE(),INTERVAL 90 DAY) and now() order by data desc , log_atualizacao desc LIMIT $registroinicial,$pagesize";
		  $dt_ini= date("d/m/Y", mktime(0, 0, 0, date("m"), date("d")-90, date("Y")));
		  $dt_fim= date("d/m/Y");
		}
		else {
		  $sql="SELECT count(*) as total from log_atualizacao where ".$whereAutorCount." data between '".seta_data($_REQUEST[dt_ini])."' and '".seta_data($_REQUEST[dt_fim])."'";
		  $sql2="SELECT a.*, b.nome FROM log_atualizacao AS a LEFT JOIN usuario AS b on (a.usuario = b.usuario) 
			WHERE ".$whereAutor." a.data between '".seta_data($_REQUEST[dt_ini])."' and '".seta_data($_REQUEST[dt_fim])."' order by data desc , log_atualizacao desc LIMIT $registroinicial,$pagesize";
		}
		  $db->query($sql);
		  $numlinhas=$db->dados();
  ...
  <tr>
    <td valign="top" class="texto"><form name="form2" method="get" action="logsistema.php">
      <div align="left">&nbsp Per&iacute;odo de busca: de
        ...
        <br>
        Obra:&nbsp;&nbsp;&nbsp;  
        <input name="num_registro" type="text" class="combo_texto" id="obra" maxlength="10" size="10" value="<? echo $_REQUEST['obra']; ?>">         
	&nbsp;&nbsp;Autor:&nbsp;
         
	<!-- pesquisa obra - PRD10 - PBL!>
 	if($_REQUEST[numregistro]<>'' )
 	{
    		$sql="SELECT * from obra where (num_registro = '$_REQUEST[numregistro]'))";
 		$db5->query($sql);
 		$obra=$db5->dados();
 	}

        <!-- monta a combo do autor PBL-->  
	   ...
	   while($res=$db4->dados())
	   {
        ...
        <a href='javascript:;' onClick="abrepopAutor('pop_autor.php');"">
        	<img src="imgs/icons/lupa.gif" title="Pesquisar..." width="27" border=0 height="16")">
        </a>
        ...
        <tr id="cor_fundo<? echo $row['log_atualizacao'] ?>" class="texto">
		  ...
		  if($row[operacao]=='E'){echo "Exclusão /";}
		   ?>
               ...
               // Exibe na lista o num_registro da obra ou o Nome do Autor PR10 - PBL

               if ($row[obra]<>"0") 
               {
                        $sqlOB="select num_registro from obra where obra=$row[obra] ";
                        $db2->query($sqlOB);
                        $obraP=$db2->dados();
              	        echo "(Obra:".$obraP[num_registro].")";
	       } else {
                        $sqlAT="select nomeetiqueta from autor where autor=$row[autor]";
                        $db2->query($sqlAT);
                        $autorP=$db2->dados();
		        echo "(".substr($autorP[nomeetiqueta],0,30).")";
               }
...
$a="<a href=\"logsistema.php?page=".$first."&dt_ini=".$_REQUEST[dt_ini]."&dt_fim=".$_REQUEST[dt_fim]."&autor=".$_REQUEST[autor]."&obra=".$_REQUEST[obra]."\"><img src='imgs/icons/btn_inicio.gif'  border='0'  alt='Registro Inicial' ></a>";

$b="<a href=\"logsistema.php?page=".$menos."&dt_ini=".$_REQUEST[dt_ini]."&dt_fim=".$_REQUEST[dt_fim]."&autor=".$_REQUEST[autor]."&obra=".$_REQUEST[obra]."\"><img src='imgs/icons/btn_anterior.gif'  border='0' alt='Registro Anterior' ></a>";

$c="<a href=\"logsistema.php?page=".$mais."&dt_ini=".$_REQUEST[dt_ini]."&dt_fim=".$_REQUEST[dt_fim]."&autor=".$_REQUEST[autor]."&obra=".$_REQUEST[obra]."\"><img src='imgs/icons/btn_proximo.gif'  border='0' alt='Proximo Registro' ></a> ";

$d="<a href=\"logsistema.php?page=".$last."&dt_ini=".$_REQUEST[dt_ini]."&dt_fim=".$_REQUEST[dt_fim]."&autor=".$_REQUEST[autor]."&obra=".$_REQUEST[obra]."\"><img src='imgs/icons/btn_ultimo.gif'  border='0' alt='Ultimo Registro' ></a>";
$combo="";
