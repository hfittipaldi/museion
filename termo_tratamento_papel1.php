...
{ 
 if($op=='update')
 {
   $sql="SELECT a.* from termo_papel_tratamento as a where a.termo_papel_tratamento='$val'";
   $db->query($sql);
   $res=$db->dados();
  }
 if($op=='del')
 {
     $sql="DELETE from termo_papel_tratamento where termo_papel_tratamento='$val'";
	 $db->query($sql);
	 echo"<script>alert('Exclusão realizada com sucesso')</script>";
	 echo"<script>location.href='termo_tratamento_papel.php'</script>";
	 exit();
  }	 
}
	 ?>
<table width="100%"  border="0" cellpadding="0" cellspacing="4">
        ...
        <tr>
          <td>
            <div align="left"><? echo "<a href=\"termo_tratamento_papel.php\"><img src='imgs/icons/btn_voltar.gif' width='20' height='20' border='0' alt='Voltar' >"?></div></td>
          <td>&nbsp;</td>
        </tr>
      </table>
      <br>
...
if($_REQUEST['enviar']<>'')
{
  if($_REQUEST[op]=='update')
   {
     $sql="UPDATE termo_papel_tratamento set termo='$_REQUEST[termo]' 
	 where termo_papel_tratamento='$_REQUEST[val]'";
	 $db->query($sql);
  ...
  //   verifica_existencia($_REQUEST[nome]);
     $sql= "INSERT INTO termo_papel_tratamento(termo) values('".trim($_REQUEST[termo])."')";
	 $db->query($sql);
