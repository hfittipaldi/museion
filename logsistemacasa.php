...
var i = qual.value;
document.location=('logsistema.php?page='+ i+ '&dt_ini=<? echo $_REQUEST[dt_ini]; ?>&dt_fim=<? echo $_REQUEST[dt_fim]; ?>&autor=<? echo trim($_REQUEST[nome]); ?>&obra=<? echo trim($_REQUEST[obra]);?>&operacao=<? echo trim($_REQUEST[operacao]) ;?>' );
}}
...
////////////
		...
		if ($_REQUEST[operacao]<>''){
		   ...
		   $WhereOperCont="";
                ...
                // Consulta autor PBL
		if ($_REQUEST[nome]<>'') {
			$whereAutor="a.obs like '%".trim($_REQUEST['nome'])."%' and ";
                        $whereAutorCount="obs like '%".trim($_REQUEST['nome'])."%' and ";
		} else {
			$whereAutor="";
                        $whereAutorCount="";
		}
		...
		if ($_REQUEST[num_registro]<>'') {
			...
			$whereObra="a.obs like '%".trim($_REQUEST['num_registro'])."%' and ";
			$whereObraCount="obs like '%".trim($_REQUEST['num_registro'])."%' and ";
  ...
  <tr>
    <td valign="top" class="texto"><form name="form2" method="get" action="logsistema.php">
      <div align="left">&nbsp Per&iacute;odo de busca: de
        ...
        &nbsp;&nbsp;Obra:&nbsp; 
        <input name="num_registro" type="text" class="combo_texto" id="obra" size="10" value="<? echo $_REQUEST['num_registro']; ?>">         
	  ...
	  <option value="I"<?if ($operacao==I) echo "selected";?>>Inclusão</option>
        ...
        &nbsp;Autor:&nbsp;
	<input name="nome" type="text" class="combo_texto" size="46" value="<?echo $_REQUEST['nome'];?>">
        ...
        <tr id="cor_fundo<? echo $row['log_atualizacao'] ?>" class="texto">
		  ...
		  if($row[operacao]=='E'){echo "Exclusão /";}
		   ?>
               ...
               { 			
                  $resp1 = "(".$row[obs].")";
		  list($ob1,$ob2,$ob3,$ob4,$ob5,$ob6,$ob7) = explode(":", $resp1);
                  $resp2 = "(".$ob5.")";
		  list($ob8,$ob9,$ob10,$ob11) = explode("=",$resp2);
                  if ($ob9<>"")
                  { echo "obra(".$ob9;}

	       } else {		 
                  $resp1 = "(".$row[obs].")";
		  list($ob8,$ob9,$ob10) = explode("=",$resp1);
                 if (($ob10<>"") & ($ob10<>")"))          		
		 { 
		    echo "(".$ob10;
				}
...
-->
          ...
          <td align="center" width='10%'><div align="center"> <! mudei (detalhes) de 15% para 10%!>
            <? echo "<a href=\"logsistema1.php?user=$row[nome]&op=$row[operacao]&obra=$row[obra]&autor=$row[autor]&obs=$row[obs]&operacao=&row[operacao]&data=".$row[data]."\">
...
$a="<a href=\"logsistema.php?page=".$first."&dt_ini=".$_REQUEST[dt_ini]."&dt_fim=".$_REQUEST[dt_fim]."&autor=".$_REQUEST[autor]."&num_registro=".$_REQUEST[num_registro]."&operacao=".$_REQUEST[operacao]."\"><img src='imgs/icons/btn_inicio.gif'  border='0'  alt='Registro Inicial' ></a>";

$b="<a href=\"logsistema.php?page=".$menos."&dt_ini=".$_REQUEST[dt_ini]."&dt_fim=".$_REQUEST[dt_fim]."&autor=".$_REQUEST[autor]."&num_registro=".$_REQUEST[num_registro]."&operacao=".$_REQUEST[operacao]."\"><img src='imgs/icons/btn_anterior.gif'  border='0' alt='Registro Anterior' ></a>";

$c="<a href=\"logsistema.php?page=".$mais."&dt_ini=".$_REQUEST[dt_ini]."&dt_fim=".$_REQUEST[dt_fim]."&autor=".$_REQUEST[autor]."&num_registro=".$_REQUEST[num_registro]."&operacao=".$_REQUEST[operacao]."\"><img src='imgs/icons/btn_proximo.gif'  border='0' alt='Proximo Registro' ></a> ";

$d="<a href=\"logsistema.php?page=".$last."&dt_ini=".$_REQUEST[dt_ini]."&dt_fim=".$_REQUEST[dt_fim]."&autor=".$_REQUEST[autor]."&num_registro=".$_REQUEST[num_registro]."&operacao=".$_REQUEST[operacao]."\"><img src='imgs/icons/btn_ultimo.gif'  border='0' alt='Ultimo Registro' ></a>";
$combo="";
