...
if ($tipo) {
  ...
  elseif($op=='delexp') {
	...
	$db->query($sql);
	echo "<script>location.href='exposicao_$tipo.php?op_obra=".$_REQUEST[op_obra]."&lista=".$lista."&".$parametro."=".$valor."';</script>";
   }
  elseif($op=='del') {
...
//////////////////////////////Tabela Log_atualizacao/////////////////////////////
        ...
        $obs1=$obs1. "Ação={Excluída exposição:".$id."}";
        $sql="insert into log_atualizacao(operacao,usuario,autor,obra,data,obs)values('A','$_SESSION[susuario]','$valor','0',now(),'$obs1')";
        $db->query($sql);
//////////////////////////////////////////////////////////////////
}
	echo "<script>location.href='exposicao_$tipo.php?op_obra=".$_REQUEST[op_obra]."&lista=".$lista."&".$parametro."=".$valor."&instituicaop=".htmlentities(str_replace("\\","",$instituicaop), ENT_QUOTES)."&nomep=".htmlentities(str_replace("\\","",$nomep), ENT_QUOTES)."';</script>";
   }
 }
...
//////////////////////////////Tabela Log_atualizacao/////////////////////////////
        ...
        $registro=$dbreg->dados(); 
        $obs1="Alteração autor ID={".$valor."}";
        $obs1=$obs1. "Ação={Inclusão exposição:".$id."}";
        $sql="insert into log_atualizacao(operacao,usuario,autor,obra,data,obs)values('A','$_SESSION[susuario]','$valor','0',now(),'$obs1')";
        $db->query($sql);
...
//////////////////////////////////////////////////////////////////
	...
	echo "<script>location.href='exposicao_insere2.php?op_obra=".$_REQUEST[op_obra]."&instituicaop=".htmlentities(str_replace("\\","",$instituicaop), ENT_QUOTES)."&nomep=".htmlentities(str_replace("\\","",$nomep), ENT_QUOTES)."&".$parametro."=".$valor."&page=".$pageID."';</script>";
	}
...
//////////////////////////////Tabela Log_atualizacao/////////////////////////////
        ...
        $registro=$dbreg->dados();
        $obs1="Alteração obra ID={".$valor."}  Registro={".$registro[num_registro]."}  Titulo="."{".trim($registro[titulo_etiq])."}";
        $obs1=$obs1. "Ação={Inclusão de nova Exposição:".$valor."}";
        $sql="insert into log_atualizacao(operacao,usuario,autor,obra,data, obs)values('A','$_SESSION[susuario]','0','$valor',now(),'$obs1')";
        $db->query($sql);
...
//////////////////////////////Tabela Log_atualizacao/////////////////////////////
        ...
        $registro=$dbreg->dados(); 
        $obs1="Alteração autor ID={".$valor."}";
        $obs1=$obs1. "Ação={Inclusão de nova exposição:".$lastid."}";
        $sql="insert into log_atualizacao(operacao,usuario,autor,obra,data,obs)values('A','$_SESSION[susuario]','$valor','0',now(),'$obs1')";
        $db->query($sql);
 ...
 ?>
         <table width="100%" height="20"  border="0" cellpadding="0" cellspacing="0">
             ...
             <tr bgcolor="#96ADBE">
                ...
                <td height="24" bgcolor="#96ADBE" class="texto_bold" style="color: white;"><div align="left"> Nova exposição a vincular <? echo $row['exposicao']; ?></div></td>
                <td ><? echo "<a href=\"exposicao_insere2.php?op_obra=".$_REQUEST[op_obra]."&instituicaop=".htmlentities(str_replace("\\","",$instituicaop), ENT_QUOTES)."&nomep=".htmlentities(str_replace("\\","",$nomep), ENT_QUOTES)."&".$parametro."=".$valor."&tipo=".$tipo."&page=".$pageID."\"'><img src='imgs/icons/btn_voltar.gif' border='0' alt='Voltar'>"; ?></td>
...
<table width="100%"  border="0" cellpadding="0" cellspacing="4"><form name="form1" method="post" onSubmit='return valida();' >
       ...
       <? if ($row['nome']==''){?>
          <td colspan="2"><input name="nome" type="text" class="combo_cadastro" id="nome" value='<? echo htmlentities(str_replace("\\","",$_REQUEST[nomep]), ENT_QUOTES); ?>' size="69"></td>
        <? }else{?>
      ...
      <? if ($row['instituicao']==''){?>
          ...
          <td colspan="2"><input name="instituicao" type="text" class="combo_cadastro" id="instituicao" value="<? echo htmlentities(str_replace("\\","",$_REQUEST[instituicaop]), ENT_QUOTES); ?>" size="69" maxlength="255"></td>
      <? }else{?>
