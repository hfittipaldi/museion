      ...
      <table width="100%" height="100%"  border="0" cellpadding="0" cellspacing="2" bgcolor="#ddddd5">
        <tr class="texto">
          <td colspan="4" align="center" class="texto"><?
           ...
           $db=new conexao();
           $db->conecta();
		   $sql="select DISTINCT uf,nome from estado ORDER by nome asc";
		   $db->query($sql);
		   echo" <select name='ufestado[]' id='ufestado' size=13 multiple class='combo_cadastro' >";
		   
		   $estado_atua= $_GET[estado_atua];
		   $estado_atua= explode(',',$estado_atua);
		   $totmembros= count($estado_atua);
		   if ($estado_atua[0] == '')
				$totmembros= 0;
		   ...
		   while($row=$db->dados())
		   {
			 ...
			 for ($i=1;$i<=$totmembros;$i++) {
				 if ($row['uf'] == $estado_atua[$i-1]) {
					 $sel= "selected";
					 break;
				 }
		   }
		     echo "<option value='$row[uf]' $sel >$row[nome] ($row[uf]) </option>";
             $res[]=$row[0];
			}
        if($_REQUEST[submit]<>'')
             {
             ...
             for($i=0;$i<count($res);$i++)
			 {
			   ...
			   if ($_REQUEST['consultaautor'] == '1')
			       echo "<script>opener.window.document.form.estadoatua.value='$k';</script>";
				else
        ...
        <tr class="texto">
          <td colspan="3" align="center" class="texto"><div align="center">Obs:Para
            selecionar mais de um estado,utilize a tecla &quot;<strong>Control</strong>&quot; </div></td>
