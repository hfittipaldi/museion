...
document.location=('obra_dimensoes_1.php?page='+ i+ '&modelo='+modelo+
'&autor=<? echo trim($_REQUEST[autor]);?>
&colecao=<? echo trim($_REQUEST[colecao]);?>
&dimIni=<? echo trim($_REQUEST[dimIni]);?>
&dimFim=<? echo trim($_REQUEST[dimFim]);?>
&dimConsulta=<? echo trim($_REQUEST[dimConsulta]);?>');
// }
}
...
<table width="540"  border="1" align="left" cellpadding="0" cellspacing="0" bgcolor="#f2f2f2">
  <tr>
    <th width="486" colspan="1" scope="col" class="tit_interno">
	    <?
		echo "<form name='form' method='post' action='obra_dimensoes.php'>";
...
if ($_REQUEST['dimConsulta'] == 1) {
	$condicao=$condicao." AND dim_obra_altura BETWEEN '$_REQUEST[dimIni]' AND '$_REQUEST[dimFim]'";
	$txtpesquisa=$txtpesquisa."<br>&nbsp;- Altura da Obra entre: <font style='color:#ffffff;'>".trim($_REQUEST['dimIni'])." e ".trim($_REQUEST['dimFim'])."</font>";

}
if ($_REQUEST['dimConsulta'] == 2) {
	$condicao=$condicao." AND dim_obra_largura BETWEEN '$_REQUEST[dimIni]' AND '$_REQUEST[dimFim]'";
	$txtpesquisa=$txtpesquisa."<br>&nbsp;- Largura da Obra entre: <font style='color:#ffffff;'>".trim($_REQUEST['dimIni'])." e ".trim($_REQUEST['dimFim'])."</font>";

}

if ($_REQUEST['dimConsulta'] == 3) {
	$condicao=$condicao." AND dim_obra_profundidade BETWEEN '$_REQUEST[dimIni]' AND '$_REQUEST[dimFim]'";
	$txtpesquisa=$txtpesquisa."<br>&nbsp;- Profundidade da Obra entre: <font style='color:#ffffff;'>".trim($_REQUEST['dimIni'])." e ".trim($_REQUEST['dimFim'])."</font>";
  ...
  //  echo "<br>";
	...
	?>
		...
		<? if (strtoupper($_SESSION[snome]) != 'VISITANTE') { ?>	
			...
			<input type="hidden" name="dimIni" value="<? echo trim($_REQUEST[dimFim]); ?>">
			<input type="hidden" name="dimFim" value="<? echo trim($_REQUEST[dimIni]); ?>">
			<input type="hidden" name="dimConsulta" value="<? echo trim($_REQUEST[dimConsulta]); ?>">
