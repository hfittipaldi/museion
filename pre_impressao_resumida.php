...
<table width="95%"  border="0">
  ...
  <tr> 
	...
	if (file_get_contents($dir.$url.'/'.$arquivo)) {
		...
		$Ao= $height;
		$Lo= $width;
		//600 é a altura max da área de exibição da imagem pequena; 600 é a largura máxima.//
		$num_alt= 600;
		$num_lar= 600;
		$cA= $Ao / $num_alt;
		$cL= $Lo / $num_lar;
...
?>
            ...
            <? if ($imagem<>'' && $noimage=='') {?>
		    <td width="97%" valign="top" align="center"><p align="center"><img src="<? echo 'http://'.$_SERVER[SERVER_NAME].'/donato/'.$dir_virtual.$url.'/'.combarra_encode($arquivo); ?>" height="<? echo $Ao; ?>" width="<? echo $Lo; ?>"></p>
            <? } else { 
			echo "<td width='97%' class='texto_bold' align='center' valign='middle' nowrap style='border: 0px dashed #ABABAB; color:#444444;'><br><em>(Imagem não disponível)</em><br><br>";
	   } ?> 
	...
	while ($lista=$db->dados()) {
		...
		echo $row[material_tecnica].", " . formata_valor_3($row['dim_obra_altura'],1,",",".") . " x " . formata_valor_3($row['dim_obra_largura']). " x " . formata_valor_3($row['dim_obra_profund']) . " cm"; 
	   ...
	   elseif ($row['dim_obra_profund'] == 0 and $row['dim_obra_diametro'] > 0 and $row['aimp_obra_diametro'] == 0 and $row['aimp_obra_altura'] > 0)
		echo $row[material_tecnica].", " . formata_valor_3($row['aimp_obra_altura']) . " x " . formata_valor_3($row['aimp_obra_largura'],1,",",".") . " cm (área impressa); ". "&Oslash; = " . formata_valor_3($row['dim_obra_diametro']) . " cm (suporte)" ; 
           else 
	...
	?>
		...
		if (strlen($dataqui) > 3)
			echo ", " . $dataqui;
		if ($row['texto_etiq'] <> '') {
			echo "<font style='font-size:8px;'><br><br><br></font><font style='font-family:times new roman,arial; font-weight:normal; font-size:14px;'><em>" . $row['texto_etiq'] . "<br></em></font>";
					}
