...
<table width="95%"  border="0">
  ...
  <tr>
	...
	$sql="SELECT a.*,b.atribuido from autor as a INNER JOIN autor_obra as b on (a.autor=b.autor) 
	...
	while ($lista=$db->dados()) {
		...
		if ($lista[dt_morte_tp] == '?')
			$mort.=" (?) ";
               if ($lista[atribuido]=="S") { $atribuido=" (atribuído)";}




		$lista_autoria .= "<b>".$lista['nomeetiqueta']."</b><i>".$atribuido."</i><br><em>".$nasc." - ".$mort.  "</em></b><br>";
	}
	...
	?>
        <!--<br>fotografia, <? /*echo formata_valor_3($row['aimp_obra_altura']) . " x " . formata_valor_3($row['aimp_obra_largura']) . " cm(área impressa); " . ret_dim_parte($row['obra']); */ ?>-->
        <br>
        <? 
 ...
 <?
    global $db2;
	$sql2 = "SELECT b.nome_arquivo,b.diretorio_imagem from fotografia_obra a, fotografia b where a.fotografia = b.fotografia
		AND a.obra = ".$obraID." order by  a.eh_mini desc, a.eh_img_laudo desc";
	$db4->query($sql2);
	$imagem = $db4->dados();
	...
	if (file_get_contents($dir.$url.'/'.$arquivo)) {
...
?>	
            ...
            <? if ($imagem<>'' && $noimage=='') { ?>
	  <td width="20%" valign="top" align="right"><img src="<? echo 'http://'.$_SERVER['SERVER_NAME'].'/donato/'.$dir_virtual.$url.'/'.combarra_encode($arquivo); ?>" height="<? echo $Ao; ?>" width="<? echo $Lo; ?>"></img></td>
            <? } else { 
			echo "<td width='30%' class='texto_bold' align='center' valign='middle' nowrap style='border: 1px dashed #ABABAB; color:#444444;'><sup>Imagem não disponível</sup></td>";
		   } ?> 
     ...
     <td height="10" colspan="2">
      ...
      <? 
					...
					echo "<font style='font-family:arial; font-weight:normal; font-size:12px;'><br><em> - Dimensões -</em><br>Altura: <b>" . number_format($row['dim_obra_altura'],2,',','.') ."</b> cm - Largura: <b>". number_format($row['dim_obra_largura'],2,',','.')."</b> cm - Diâmetro: <b>". number_format($row['dim_obra_diametro'],2,',','.') . "</b> cm - Profundidade: <b>". number_format($row['dim_obra_profund'],2,',','.') ."</b> cm";
					echo "<font style='font-family:arial; font-weight:normal; font-size:12px;'><br>Peso: <b>" . number_format($row['dim_obra_peso'],2,',','.') ."</b> kg - Formato: <b>";
					...
					$area_impressa= "<br><br><em>- Dimensões da área impressa -</em><br>Altura: <b>" . number_format($row['aimp_obra_altura'],2,',','.') ."</b> cm - Largura: <b>". number_format($row['aimp_obra_largura'],2,',','.')."</b> cm - Diâmetro: <b>". number_format($row['aimp_obra_diametro'],2,',','.') ."</b> cm - Formato: <b>". $formato_area_impressa."</b><br>";
					 }
					echo $area_impressa; 

					echo "<font style='font-family:arial; font-weight:normal; font-size:12px;'><br>Descrição de conteúdo: <b>".$desc_conteudo."</b><br>";

					echo "<font style='font-family:arial; font-weight:normal; font-size:12px;'><br>Temas: <b>" . ret_temas($row[obra])."</b><br>";
			...
			/*		$dt_aq_di= $row['dt_aquisicao_di']."-";
					...
					echo "<font style='font-family:arial; font-weight:normal; font-size:12px;'><br>Ex-proprietários: <b>" . $row['ex_proprietarios'] ."</b><br>";

					  $sql="SELECT nome from local where local='$row[local_fixo]'";
					  $db2->query($sql);
					  $res2=$db2->dados();

					echo "<font style='font-family:arial; font-weight:normal; font-size:12px;'><br>Localização fixa: <b>" . $res2[0] ."</b><br>";
					...
					echo "<font style='font-family:arial; font-weight:normal; font-size:12px;'><br>Observações: <b>" . $row['obs'] ."</b><br>";
					echo "<font style='font-family:arial; font-weight:normal; font-size:12px;'><br>Texto para etiqueta: <b>" . $row['texto_etiq'] ."</b><hr>";
					...
					while ($par=$db2->dados()) {
						...
						"Altura: <b>" . number_format($par['dim_aimp_altura'],2,',','.')." </b>cm - Largura: <b>".number_format($par['dim_aimp_largura'],2,',','.')." </b>cm - Diâmetro: <b>".number_format($par['dim_aimp_diametro'],2,',','.')." </b>cm - Formato: <b>".number_format($parte_aimp_formato,2,',','.')."</b><br><br>";
						 }


						if ($par['dim_mold_possui'] == 'S') {
							$parte_mold= "<b> SIM </b><br> Altura: <b>" .number_format($par['dim_mold_altura'],2,',','.')." </b>cm - Largura: <b>".number_format($par['dim_mold_largura'],2,',','.')." </b>cm - Diâmetro: <b>".number_format($par['dim_mold_diametro'],2,',','.')." </b>cm - Profundidade: <b>".number_format($par['dim_mold_profund'],2,',','.')."</b> - Peso: <b>".number_format($par['dim_mold_peso'],2,',','.')." </b>kg<br>";
						}
						else {
							$parte_mold= "<b> NÃO</b><br>";
						 }
						if ($par['dim_base_possui'] == 'S') {
							$parte_base= "<b> SIM </b><br> Altura: <b>" . number_format($par['dim_base_altura'],2,',','.')." </b>cm - Largura: <b>".number_format($par['dim_base_largura'],2,',','.')." </b>cm - Diâmetro: <b>".number_format($par['dim_base_diametro'],2,',','.')." </b>cm - Profundidade: <b>".number_format($par['dim_base_profund'],2,',','.')."</b> - Peso: <b>".number_format($par['dim_base_peso'],2,',','.')." </b>kg<br>";
						}
						else {
							$parte_base= "<b> NÃO</b><br>";
						 }
						if ($par['dim_pasp_possui'] == 'S') {
							$parte_pasp= "<b> SIM </b><br> Altura: <b>" . number_format($par['dim_pasp_altura'],2,',','.')." </b>cm - Largura: <b>".number_format($par['dim_pasp_largura'],2,',','.')." </b>cm - Diâmetro: <b>".number_format($par['dim_pasp_diametro'],2,',','.')." </b>cm - Profundidade: <b>".number_format($par['dim_pasp_profund'],2,',','.')."</b> - Peso: <b>".number_format($par['dim_pasp_peso'],2,',','.')." </b>kg";
						}
						else {
							$parte_pasp= "<b> NÃO</b>";
						 }
						...
						- Dimensões - <br></em>".
						"Altura: <b>" . number_format($par['dim_parte_altura'],2,',','.')." </b>cm - Largura: <b>".number_format($par['dim_parte_largura'],2,',','.')." </b>cm - Diâmetro: <b>".number_format($par['dim_parte_diametro'],2,',','.')." </b>cm - Profundidade: <b>".number_format($par['dim_parte_profund'],2,',','.')."</b> cm<br>Peso: <b>".number_format($par['dim_parte_peso'],2,',','.')." </b>kg - Formato: <b>".$parte_formato."</b><br><br>".
						$dim_parte.
						"Moldura: ".$parte_mold."<br>".
						"Base: ".$parte_base."<br>".
						"Passe partout: ".$parte_pasp."<hr>";

						}	
						echo "<font style='font-family:arial; font-weight:normal; font-size:12px;'>" . $parte ."</font>";
						echo "<font style='font-family:arial; font-weight:normal; font-size:12px;'><em><b> AUTORIA </b><br></em>" . $lista_autoria . "</font><hr>";

	
//	  $db->query($sql3);


					$sql="SELECT b.referencia,b.autoria,b.local,b.editora,b.sub_titulo,b.notas,a.observacao,b.ano from obra_bibliografia as a inner join bibliografia as b on (a.bibliografia=b.bibliografia) 
					...
					while ($bib=$db2->dados()) {
						...
						if ($ano_bib == 0)
			        		...
			        		if ($sub_tit_bib != '')
							$sub_tit_bib= $bib['sub_titulo'].". ";
						$editora_bib= $bib['editora'];
						if ($editora_bib != '')
							$editora_bib= $bib['editora'].". ";
						$notas_bib= $bib['notas'];
						if ($notas_bib != '')
							$notas_bib= $bib['notas'].". ";
						$bibliografia .= "- ".$bib['autoria'].". <b><em>".$bib['referencia'] ."</em></b>. ".$sub_tit_bib.$bib['local'].": ".$editora_bib.$ano_bib.". ".$notas_bib.$bib['observacao'].  ". <br>";
					}
						echo "<font style='font-family:arial; font-weight:normal; font-size:12px;'><em><b> REFERÊNCIAS BIBLIOGRÁFICAS<br></b></em>" . $bibliografia . "</font><hr>";
					...
					while ($exp=$db2->dados()) {
						...
						$exposicao .= "- "."<b>".$exp['nome']."</b>". ",&nbsp;" . $exp['instituicao'] . ",&nbsp;" .$exp['cidade'].$estado.", &nbsp;". $pais.". &nbsp;".$exp['periodo'] . ".&nbsp;" ."<em>".$exp['premio']. "</em><br>";
							...
							echo "<font style='font-family:arial; font-weight:normal; font-size:12px;'><em><b> EXPOSIÇÕES<br></em></b>" . $exposicao . "</font><hr>";
			...
			if ($dtrete=='00/00/0000' || $dtrete=="//")
				$dtrete= "--/--/----";
					...
					echo "<font style='font-family:arial; font-weight:normal; font-size:12px;'><hr>Catalogado por <b>" . $catalogado ."</b> em <b>".formata_data($row[data_catalog1]) ."</b><br>";
					echo "<font style='font-family:arial; font-weight:normal; font-size:12px;'>Atuzalizado por <b>" . $atualizado ."</b> em <b>".formata_data($row[data_catalog2]) ."</b>";
