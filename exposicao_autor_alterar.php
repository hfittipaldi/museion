...
if($_REQUEST['enviar']<>'') {
  if($op == 'update') {
	...
	$db->query($sql);
        ...
        $sql2="select count(*) as total from autor_exposicao where autor=".$valor." and exposicao=".$id;
        $db->query($sql2);
        $cont=$db->dados();
        if ($cont[total] > 0 )
        {
...
//////////////////////////////Tabela Log_atualizacao/////////////////////////////
        ...
        $registro=$dbreg->dados(); 
        $obs1="Alteração autor ID={".$valor."}";
        $obs1=$obs1. "Ação={Alteração exposição:".$id."}";
        $sql="insert into log_atualizacao(operacao,usuario,autor,obra,data,obs)values('A','$_SESSION[susuario]','$valor','0',now(),'$obs1')";
        $db->query($sql);
//////////////////////////////////////////////////////////////////
	    ...
	    $sql2="insert into autor_exposicao (exposicao, autor, premio) values ('$id', '$valor', '$_REQUEST[premio]') ";
	    $db->query($sql2); 
//////////////////////////////Tabela Log_atualizacao/////////////////////////////
        $sqlreg="SELECT nomeetiqueta FROM autor WHERE autor='$valor'";
        $dbreg->query($sqlreg);
...
//////////////////////////////////////////////////////////////////
	...
	echo "<script>location.href='exposicao_insere2.php?instituicaop=".htmlentities(str_replace("\\","",$instituicaop), ENT_QUOTES)."&nomep=".htmlentities(str_replace("\\","",$nomep), ENT_QUOTES)."&".$parametro."=".$valor."&page=".$pageID."';</script>";
...
?>
          <table width="100%" height="20"  border="0" cellpadding="0" cellspacing="0">
             ...
             <tr bgcolor="#96ADBE">
                ...
                <td ><? echo "<a href=\"exposicao_insere2.php?instituicaop=".htmlentities(str_replace("\\","",$instituicaop), ENT_QUOTES)."&nomep=".htmlentities(str_replace("\\","",$nomep), ENT_QUOTES)."&".$parametro."=".$valor."&tipo=".$tipo."&page=".$pageID."\"'><img src='imgs/icons/btn_voltar.gif' border='0' alt='Voltar'>"; ?></td>
