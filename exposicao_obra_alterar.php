...
if ($tipo <>'') {
  ...
  elseif($op=='delexp') {
	...
	$db->query($sql);
	echo "<script>location.href='exposicao_obra.php?op_obra=".$_REQUEST[op_obra]."&".$parametro."=".$valor."';</script>";
   }
  elseif($op=='del') {
...
if($_REQUEST['enviar']<>'') {
  if($op == 'update') {
	...
	$db->query($sql);
        ...
        $sql2="select count(*) as total from obra_exposicao where obra=".$valor." and exposicao=".$id;
        $db->query($sql2);
        $cont=$db->dados();
        if ($cont[total] > 0 )
        {
	    ...
	    $sql2="insert into obra_exposicao (exposicao, obra, premio) values ('$id', '$valor', '$_REQUEST[premio]') ";
	    $db->query($sql2); 
        }
