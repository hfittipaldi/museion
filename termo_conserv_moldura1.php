...
{ 
 if($op=='update')
 {
   $sql="SELECT a.* from termo_moldura_estado as a where a.termo_moldura_estado='$val' order by termo";
   $db->query($sql);
   $res=$db->dados();
  }
 if($op=='del')
 {
     $sql="DELETE from termo_moldura_estado where termo_moldura_estado='$val'";
	 $db->query($sql);
	 echo"<script>alert('Exclusão realizada com sucesso')</script>";
	 echo"<script>location.href='termo_conserv_moldura.php'</script>";
	 exit();
  }	 
}
	 ?>
<table width="100%"  border="0" cellpadding="0" cellspacing="4">
        ...
        <tr>
          <td>
            <div align="left"><? echo "<a href=\"termo_conserv_moldura.php\"><img src='imgs/icons/btn_voltar.gif' width='20' height='20' border='0' alt='Voltar' >"?></div></td>
          <td>&nbsp;</td>
        </tr>
      </table>
      <br>
...
if($_REQUEST['enviar']<>'')
{
  if($_REQUEST[op]=='update')
   {
     $sql="UPDATE termo_moldura_estado set termo='$_REQUEST[estado_conserva]' 
	 where termo_moldura_estado='$_REQUEST[val]'";
	 $db->query($sql);
  ...
  //   verifica_existencia($_REQUEST[nome]);
     $sql= "INSERT INTO termo_moldura_estado(termo) values('".trim($_REQUEST[estado_conserva])."')";
	 $db->query($sql);
