...
if($_REQUEST[procurar]<>'')
{
 ...
 $res=$db->dados();
  ...
  if($conta==1)
	    echo"<script>location.href='cadastrobrareg.php?op=update&obra=$res[obra]'</script>";
  if($conta>1){
   echo "<script>location.href='obra_ocorrencia_altera.php?titulo=$_REQUEST[titulo]&numregistro=$_REQUEST[numregistro]'</script>"; }
 }
}
...
if($_REQUEST[catal]<>'')
{
  ...
  if($conta==0) { 
   ...
   echo "<script>location.href='obra_ocorrencia_altera.php?emcatalog=1'</script>"; }
 }
