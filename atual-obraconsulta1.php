...
if ($_REQUEST['dim_obra_altura_ini'] <> '' || $_REQUEST['dim_obra_altura_fim'] <> '') {
	$condicao=$condicao." AND dim_obra_altura BETWEEN '$_REQUEST[dim_obra_altura_ini]' and '$_REQUEST[dim_obra_altura_fim]'";
...
if ($_REQUEST['dim_obra_largura_ini'] <> '') {
	$condicao=$condicao." AND dim_obra_largura BETWEEN '$_REQUEST[dim_obra_largura_ini]' and '$_REQUEST[dim_obra_largura_fim]'";
...
if ($_REQUEST['dim_obra_diametro_ini'] <> ''){
	$condicao=$condicao." AND dim_obra_diametro BETWEEN '$_REQUEST[dim_obra_diamentro_ini]'and '$_REQUEST[dim_obra_diametro_fim]'";
	$txtpesquisa=$txtpesquisa."<br>&nbsp;- Diâmetro da Obra: <font style='color:#ffffff;'>".trim($_REQUEST['dim_obra_diametro'])."</font>";

}
if ($_REQUEST['dim_obra_profundidade_ini'] <> '') {
	$condicao=$condicao." AND dim_obra_profund BETWEEN '$_REQUEST[dim_obra_profundidade_ini]' and '$_REQUEST[dim_obra_profundidade_fim]'";
	$txtpesquisa=$txtpesquisa."<br>&nbsp;- Profundidade da Obra: <font style='color:#ffffff;'>".trim($_REQUEST['dim_obra_profund'])."</font>";

}
if ($_REQUEST['dim_obra_peso_ini'] <> '') {
	$condicao=$condicao." AND dim_obra_peso BETWEEN '$_REQUEST[dim_obra_peso_ini]' and '$_REQUEST[dim_obra_peso_fim]'";
...
if ($_REQUEST['aimp_obra_altura_ini'] <> '') {
	$condicao=$condicao." AND aimp_obra_altura BETWEEN '$_REQUEST[aimp_obra_altura_ini]' and '$_REQUEST[aimp_obra_altura_fim]'";
...
if ($_REQUEST['aimp_obra_largura_ini'] <> '') {
	$condicao=$condicao." AND aimp_obra_largura BETWEEN '$_REQUEST[aimp_obra_largura_ini]' and '$_REQUEST[aimp_obra_largura_fim]'";
...
if ($_REQUEST['aimp_obra_diametro_ini'] <> '') {
	$condicao=$condicao." AND aimp_obra_diametro BETWEEN '$_REQUEST[aimp_obra_diametro_ini]' and '$_REQUEST[aimp_obra_diametro_fim]'";
	$txtpesquisa=$txtpesquisa."<br>&nbsp;- Diâmetro da &Aacute;rea Impressa: <font style='color:#ffffff;'>".trim($_REQUEST['aimp_obra_diametro'])."</font>";
  ...
  //  echo "<br>";
	...
	?>
		...
		<? if (strtoupper($_SESSION[snome]) != 'VISITANTE') { ?>	
			...
			<a href="javascript:;" style="text-decoration: none;" onClick="abrepop3('pre_impressao_lista.php');"><sub><img src="imgs/icons/ic_lista.gif" border="0" title="Gravar/imprimir lista de registros sem foto"></sub></a>
