...
<html>
<head>
<title><? echo str_replace('\"','"',htmlentities($row['num_registro'], ENT_QUOTES)); ?></title><body class="combo_cadastro" onload='esconde()' >
...
document.location=('consulta_obra_2.php?page='+ i+ '&modelo='+modelo+
...
function abre_print(idobra,title)
{
  	win=window.open('pre_impressao_obras_nova.php?op=view&nosave=1&titulo='+title+'&obra='+idobra,'PAG','left='+((window.screen.width/2)-390)+',top='+((window.screen.height/2)-240)+',height=480,width=780,scrollbars=yes,status=no,toolbar=no,menubar=no,location=no', screenX=0, screenY=0);
 if(parseInt(navigator.appVersion)>=4){
   win.window.focus();
 }
}
function abre_print2(idobra,title)
{
  	win=window.open('pre_impressao_resumida.php?op=view&nosave=1&titulo='+title+'&obra='+idobra,'PAG','left='+((window.screen.width/2)-390)+',top='+((window.screen.height/2)-240)+',height=480,width=780,scrollbars=yes,status=no,toolbar=no,menubar=no,location=no', screenX=0, screenY=0);
 if(parseInt(navigator.appVersion)>=4){
   win.window.focus();
 }
}
...
<table width="760"  border="1" align="left" cellpadding="0" cellspacing="0" bgcolor="#f2f2f2">
  <? 
...
if ($_REQUEST['num_registro'] <> '') {
	...
	$txtpesquisa=$txtpesquisa."<br>&nbsp;Nº de registro: <font style='color:brown;'>".$_REQUEST[num_registro]."</font>";
}
if ($_REQUEST['obra'] <> '') {
	$condicao=$condicao." AND (O.obra = $_REQUEST[obra])";
	$txtpesquisa=$txtpesquisa."<br>&nbsp;Obra: <font style='color:brown;'>".$_REQUEST[obra]."</font>";
}
...
?>
  <tr>
    <td width="876" colspan="2" valign="top"></td>
  </tr>
  <tr>
...
//   Salva o SQL em outra variavel para consulta pelo numero do registro
    $sqlPesq="SELECT num_registro $from where status='P' $condicao order by num_registro + 0 asc";
...
//  		$sql2="SELECT $select2 $from where status='P' and (B.hierarquia = 1 or B.hierarquia IS null) $condicao order by  titulo asc LIMIT $registroinicial,$pagesize";
  ...
  //  echo "<br>";
      ...
      <table width="100%" height="20"  border="0" cellpadding="0" cellspacing="0">
        ...
        <tr bgcolor="#96ADBE">
          <td height="20" bgcolor="#ddddd5" valign="middle" class="texto_bold"><div align="center" >&nbsp;<? echo "Ficha técnica"; ?></span>
	  </td>
        </tr>
        <tr bgcolor="#f2f2f2">
        </tr>
        ...
        <table width="97%" height="100%"  border="0" cellpadding="0" cellspacing="6" bgcolor="#f2f2f2">
          <? while($row=$db->dados())
	  {
				...
				$sql="SELECT a.nome_arquivo,a.diretorio_imagem,a.forma_exibicao,b.eh_principal from fotografia as a, fotografia_obra as b 
					   where a.fotografia = b.fotografia AND b.obra = '$row[obra]' order by b.eh_mini desc";
				$db3->query($sql);
				$dim= $db3->dados();
	  ...
	  ?>
          <tr class="texto"> 
            ...
            <!--<td width="30%" valign="middle" align="center" nowrap class="texto"><a href="javascript:;" onClick="abrepop('pop_imagem.php?exibicao=<? echo $forma_exibicao; ?>&principal=<? echo $principal; ?>&imagem=<? echo $url[0].'/'.$imagem; ?>&altura=<? echo $altu; ?>&largura=<? echo $larg; ?>&diametro=<? echo $diam; ?>&profundidade=<? echo $prof; ?>', <? echo $height; ?>, <? echo $width; ?>);"><img src='<? echo $dir_virtual.$url[0].'/'.combarra_encode($imagem); ?>' height="<? echo $Ao; ?>" width="<? echo $Lo; ?>" border='0'></a></td>-->
				<td width="17%" valign="middle" align="center" nowrap class="texto"><a href="javascript:;" onClick="abrepop4('imagem_lista.php?obra=<? echo $row[obra]; ?>');"><img src='<? echo $dir_virtual.$url[0].'/'.combarra_encode($imagem); ?>' height="<? echo $Ao; ?>" width="<? echo $Lo; ?>" border='0'></a></td>
            <? } else { 
            ...
            <td width="83%" align="justify" valign="top" class="texto_bold" style="font-weight:normal;"> 
	...
	?>
		...
		<tr  font-weight:normal;" valign="top" class="texto_bold">
			<td valign="top" align="right">&nbsp;<em>Descrição de conteúdo:</em></td>
			<td><textarea name='descri' id="descri" class="combo_cadastro rolagem" readonly style=" border:1px solid #ABABAB;" cols='110' rows='4'><? echo $row['desc_conteudo']; ?></textarea></td>
		</tr>
		...
		?>
		<tr " valign="top" class="texto_bold">
			<td valign="top" align="right">&nbsp;<em>Exposições:</em></td>
			<td><textarea name='exposic' id="exposic" class="combo_cadastro rolagem" readonly style=" border:1px solid #ABABAB;" cols='110' rows='4'><? echo str_replace("|||","\n",$exposicao); ?></textarea></td>
		</tr>
		...
		?>
		<tr  font-weight:normal;" valign="top" class="texto_bold">
			<td valign="top" align="right">&nbsp;<em>Bibliografia:</em></td>
			<td><textarea name='biblio' id="biblio" class="combo_cadastro rolagem" readonly style=" border:1px solid #ABABAB;" cols='110' rows='4'><? echo str_replace("|||","\n",$bibliografia); ?></textarea></td>
		...
		<? if (strtoupper($_SESSION[snome]) != 'VISITANTE') {?>	                    
			<td colspan="2" nowrap class="texto" align="center"> 
			<a href="javascript:;" style="text-decoration: none;"  onClick="abre_pagina('<? echo $row[obra]; ?>','<? echo htmlentities(str_replace("'","`",$row[titulo_etiq]), ENT_QUOTES); ?>');"><sub><img src="imgs/icons/busca.gif" border="0" title="Abrir FICHA COMPLETA"></sub></a>
			<a href="javascript:;" style="text-decoration: none;" onClick="abre_print('<? echo $row[obra]; ?>','<? echo htmlentities(str_replace("'","`",$row[titulo]), ENT_QUOTES); ?>');"><sub><img src="imgs/icons/ic_salvar_impressao.gif" border="0" title="Imprimir/gravar"></sub></a>
			...
			<input type="hidden" name="num_registro" value="<? echo trim(htmlentities($_REQUEST[num_registro], ENT_QUOTES)); ?>">
