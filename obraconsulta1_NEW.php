...
function abrepopetiq(janela)
{
  ...
  win=window.open(janela,'lista','left='+((window.screen.width/2)-250)+',top='+((window.screen.height/2)-250)+',width=600,height=500, scrollbars=no, resizable=no');
 if(parseInt(navigator.appVersion)>=4)
   {      win.window.focus();
    }
 return true;
...
if ($_REQUEST['num_registro_de'] <> '' and $_REQUEST['num_registro_ate']<>'' ) {
  	$condicao=$condicao." AND (O.num_registro between ".$_REQUEST['num_registro_de']." and ".$_REQUEST['num_registro_ate'].")";
	$txtpesquisa=$txtpesquisa."<br>&nbsp;- Nº de registro de: <font style='color:brown;'>".trim($_REQUEST['num_registro_de'])." até ".trim($_REQUEST['num_registro_ate'])."</font>";
	$txtpesquisa_rel=$txtpesquisa_rel."- Nº de registro de: ".trim($_REQUEST['num_registro_de'])." até ".trim($_REQUEST['num_registro_ate']);
...
<tr><td colspan="2" align="left" nowrap class="texto" style="border-top: 1px solid #96ADBE;">
		      &nbsp;Digite o Nº de Registro:&nbsp;<input type=text name=NumeroReg size="12" class="combo_texto">
      <?
