...
function valida()
{
 with(document.form1){
    if(nome.value==''){
	  alert('Preencha com o vínculo da foto.');
	  nome.focus();
	  return false;}
 }
}
...
$db=new conexao();
$db->conecta();
$val=base64_decode($_REQUEST['vinculofoto']);
$op=$_REQUEST['op'];
echo $_SESSION['lnk'];
?>
</div></th>
...
{ 
 if($op=='update')
 {
   $sql="SELECT nome from vinculo_foto as a where a.vinculo_foto='$val'";
   $db->query($sql);
   $res=$db->dados();
  }
 if($op=='del')
 {
     $sql="DELETE from vinculo_foto where vinculo_foto='$val'";
	 $db->query($sql);
	 echo"<script>alert('Exclusão realizada com sucesso')</script>";
	 echo"<script>location.href='vinculofoto.php'</script>";
	 exit();
  }	 
}
	 ?>
<table width="100%"  border="0" cellpadding="0" cellspacing="4">
        <tr>
          <td colspan="2" class="texto_bold"><div align="center"><br><br><br>Tipo: &nbsp;              
              <input name="nome" type="text" class="combo_cadastro" id="nome" value="<? echo htmlentities($res[0], ENT_QUOTES); ?>" size="77">
          </div></td>
        </tr>
        <tr>
          <td colspan="2">
        ...
        <tr>
          <td>
            <div align="left"><? echo "<a href=\"vinculofoto.php\"><img src='imgs/icons/btn_voltar.gif' width='20' height='20' border='0' alt='Voltar' >"?></div></td>
          <td>&nbsp;</td>
        </tr>
      </table>
      <br>
...
if($_REQUEST['enviar']<>'')
{
  if($_REQUEST[op]=='update')
   {
     $sql="UPDATE vinculo_foto set nome='$_REQUEST[nome]' where vinculo_foto='$_REQUEST[val]'";
	 $db->query($sql);
...
/*     $sql= "INSERT INTO vinculo_foto(vinculo_foto,nome) values('','".trim($_REQUEST[nome])."')";*/
     $sql= "INSERT INTO vinculo_foto(nome) values('".trim($_REQUEST[nome])."')";
	 $db->query($sql);
