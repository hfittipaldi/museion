...
<? 
	...
	$dir_virtual= diretorio_virtual();
        ...
        $tridimensional=$_REQUEST[tridimensional];
...
{
 global $db2;
 $sql="SELECT dt_parte_ano1,dt_parte_ano2,dt_parte_tp,assinada,transc_assinatura from parte where obra='$obrid' order by controle";
 $db2->query($sql);
 $res=$db2->dados();
...
//########### PEGA A SEQUÊNCIA DE ID DA OBRA #################################
...
//########### SE FOR POR ETAPAS ORDENA A SEQUÊNCIA PELO NÚMERO DO REGISTRO  #################################
...
if ($etapas<>''and ($etapas<>'/Todas'))
   {   
       $id_obras_t="";
	$sql1= "SELECT * from obra where obra in ($id_obras) order by num_registro + 0";
        $db1->query($sql1);
        while($row1=$db1->dados())
        {
         if ($id_obras_t=="") {
              $id_obras_t=$row1[obra].",";
         }else{
            $id_obras_t=$id_obras_t.$row1[obra].",";
         }

        }
       $id_obras="";
       if ($id_obras_t<>""){$id_obras_t=$id_obras_t."-1";}
       $id_obras=$id_obras_t;
       ...
       {
        $id_obras_temp=$id_obras_temp;
        if ($id_obras_str[$i]<>'')
        {
           if ($id_obras_temp==''){ $id_obras_temp=$id_obras_str[$i];}else{$id_obras_temp=$id_obras_temp.",".$id_obras_str[$i];}
        }
       }
      
      if (trim($id_obras_temp)<>'') {
	   $sql= "SELECT * from obra where obra in ($id_obras_temp) order by num_registro + 0";
	   $db->query($sql);
 
      }else{
           echo"<script>alert('Não existe obra tridimensional na seleção relaizada.');</script>";
...
<table width="95%"  border="0">
      ...
      <? if ($row['dim_obra_profund'] > 0 and $row['dim_obra_diametro'] == 0 and $row['aimp_obra_diametro'] == 0 and $row['aimp_obra_altura'] == 0){
        ...
        <br>
        <? 
		if (trim($p_ass) == 'S')  { echo "assinada <em>".$p_assinatura."</em>"; } else  { echo "sem assinatura"; } 
 ...
 <?
	...
	$sql2="SELECT b.nome_arquivo,b.diretorio_imagem from fotografia_obra as a, fotografia as b where a.fotografia = b.fotografia
		AND a.obra = ".$obraID."  order by  a.eh_mini desc, eh_principal desc, eh_img_laudo desc";
	$db4->query($sql2);
	$imagem=$db4->dados();
	 ...
	 <? if ($comimagem=='false')
                 {
                      ...
                      {           
                          $sq_trid= "SELECT * from obra where obra='".$obraID."'";
	                  $db_trid->query($sq_trid);
      ...
      <? if ($modeloR<>'1') {                   
          if ($modeloR=='2'  || $modeloR=='4'){
             if ($row['desc_conteudo'] <> '')  {
   		   echo "<tr></td><td width='90%'><ol><font style='font-family:arial; font-weight:normal; font-size:$tf;'><br><em><b>DESCRIÇÃO:<br></em></b>" . $row['desc_conteudo'] . "<br></em></font></td></tr>";
	     ...
	     $exposicao= "";
	     $linhas=$linhas+2;
	     echo "<tr><td width='90%'><ol><font style='font-family:arial; font-weight:normal; font-size:$tf;'><br><em><b>EXPOSIÇÕES:<br></em></b></font></td></tr>";
	     while ($exp=$db2->dados()){
                if ($exp['exposicao']<>''){  
                   $pais=$exp['pais'];
		   ...
		   echo "<tr><td width='90%'><ol><font style='font-family:arial; font-weight:normal; font-size:$tf;'>".$exposicao."</font></td></tr>";
	      ...
	      //if ($exposicao <> '')  echo "<font style='font-family:arial; font-weight:normal; font-size:$tf;'><br><em><b>EXPOSIÇÕES:<br></em></b>" . $exposicao . "</font>";	       
           ...
           if ($modeloR=='3'  || $modeloR=='4'){ 
              ...
              $sql="SELECT b.bibliografia, b.referencia,b.sub_titulo,b.autoria,b.local,b.editora,b.notas,a.observacao,b.ano from obra_bibliografia as a inner join bibliografia as b on (a.bibliografia=b.bibliografia) 
	            where a.obra=$row[obra] order by b.ano asc";
	      $db2->query($sql);
	      $bibliografia= "";
	      $linhas=$linhas+2;
	      echo "<tr><td width='90%'><ol><font style='font-family:arial; font-weight:normal; font-size:$tf;'><em><b><br>REFERÊNCIAS BIBLIOGRÁFICAS:<br></b></em></font></td></tr>";
	      while ($bib=$db2->dados()){
                 if ($bib['bibliografia'] <>'' ){
                   $ano_bib= $bib['ano'];
	            if ($ano_bib == 0)
		    ...
		    $bibliografia = "- ".$bib['autoria'].". <b><em>".$bib['referencia'] ."</em></b>. ".$sub_tit_bib .$bib['local'].": ".$bib['editora'].", ".$ano_bib.". ".$notas_bib.$bib['observacao'].  ". <br>";
                    ...
                    echo "<tr><td width='90%'><ol><font style='font-family:arial; font-weight:normal; font-size:$tf;'>".$bibliografia."</font></td></tr>";
	      ...
	      //if ($bibliografia <> '') echo "<font style='font-family:arial; font-weight:normal; font-size:$tf;'><em><b><br>REFERÊNCIAS BIBLIOGRÁFICAS:<br></b></em>" . $bibliografia . "</font>";
              ...
              if ($row['texto_etiq'] <> '') {
			echo "<tr><td width='90%'><ol><font style='font-size:8px;'><br><br></font><font style='font-family:times new roman,arial; font-weight:normal; font-size:14px;'><em>" . $row['texto_etiq'] . "<br></em></font></td></tr>";
		        $linhas=$linhas + (strlen($row['texto_etiq'])/100)+1;
            ...
            if ($modeloR=='5'){                     
	       if ($seguro <> ''){
                  	echo "<tr><td width='90%'><ol><font style='font-family:arial; font-weight:normal; font-size:$tf;'><em>Valor do Seguro:" ."&nbsp;". $seguro . "</em></font></td></tr>";
