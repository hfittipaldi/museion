...
var i = qual.value;
document.location=('obra_ocorrencia_altera.php?page='+ i+ '&titulo=<? echo $_REQUEST[titulo]; ?>&numregistro=<? echo $_REQUEST[numregistro]; ?>&emcatalog=<? echo $_REQUEST[emcatalog]; ?>');
}}

function posiciona(valor) {
var i = valor;
document.location=('obra_ocorrencia_altera.php?page='+ i+ '&titulo=<? echo $_REQUEST[titulo] ?>&vinculo=<? echo $_REQUEST[vinculo] ?>&funcao=<? echo $_REQUEST[funcao] ?>&cor=<? echo $_REQUEST[cor] ?>');
}
...
?>
      ...
      <table width="100%" height="100%"  border="0" cellpadding="0" cellspacing="2">
		<? while($row=$db->dados())
	  {  $sqlautor="SELECT a.nomeetiqueta FROM autor a inner join autor_obra as b WHERE (a.autor = b.autor) and b.obra='".$row[obra]."'";
             $dbautor->query($sqlautor);
             $rowautor=$dbautor->dados();
	  ?>
        <tr class="texto" id="cor_fundo<? echo $row['obra'] ?>">
          <td height="24" colspan="3" align="justify"><? echo "<b>".$row[num_registro]."</b>"." - ".$rowautor[nomeetiqueta]." - ".$row[titulo] ?><div align="left">
            </div></td>
          <td align="center" width='15%'>
            <div align="center"><? echo "<a href=\"alterarobra.php?op=update&obra=".$row[obra]."\">
