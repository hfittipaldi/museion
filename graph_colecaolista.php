...
document.location=('graph_colecaolista.php?page='+i);
}}

function posiciona(valor) {
var i = valor;
document.location=('graph_colecaolista.php?page='+ i+ '&titulo=<? echo $_REQUEST[titulo] ?>&vinculo=<? echo $_REQUEST[vinculo] ?>&funcao=<? echo $_REQUEST[funcao] ?>&cor=<? echo $_REQUEST[cor] ?>');
}

function abre_grafico(p1,p2,p3)
{
  	win=window.open('graphcolecao.php?p1='+p1+'&p2='+p2+'&p3='+p3,'Grafico','left='+((window.screen.width/2)-370)+',top='+((window.screen.height/2)-225)+',scrollbars=no, height=450,width=740,status=no,toolbar=no,menubar=no,location=no');
  ...
  <tr>
    ...
    <?
	  ...
	  $sql="SELECT count(*) T  FROM colecao a, obra b
            WHERE (a.colecao = b.colecao)group by b.colecao ORDER BY a.nome";
	  $db->query($sql);
      ...
      WHERE (a.colecao = b.colecao) GROUP BY b.colecao ORDER BY a.nome   LIMIT $registroinicial,$pagesize";
	 ...
	 ?>
      <table width="100%" height="20"  border="0" cellpadding="0" cellspacing="0">
        ...
        <tr bgcolor="#ddddd5">
          <td width="44%" height="24" bgcolor="#ddddd5" class="texto_bold">&nbsp;&nbsp;Coleção</td>
		...
		<? while($row=$db->dados())
	  {
        ...
        <tr class="texto" id="cor_fundo<? echo $row[obra] ?>">
          <td align="justify" height="20"><b>&nbsp;<? echo $row[nome] ?></b> 
		  </td>
          <td align="center" width='14%'><? // nao passar op=view senao fara insert em log_pesquisa 
	 echo "<a href=\"javascript:;\" OnClick=\"abre_grafico(".$tot_geral[0].",".$row[T].",'".$row[nome]."')\" > 
...
$a="<a href=\"graph_colecaolista.php?page=".$first."\"><img src='imgs/icons/btn_inicio.gif'  border='0'  alt='Registro Inicial'></a>";

$b="<a href=\"graph_colecaolista.php?page=".$menos."\"><img src='imgs/icons/btn_anterior.gif'  border='0' alt='Registro Anterior'></a>";

$c="<a href=\"graph_colecaolista.php?page=".$mais."\"><img src='imgs/icons/btn_proximo.gif'  border='0' alt='Proximo Registro'></a>";

$d="<a href=\"graph_colecaolista.php?page=".$last."\"><img src='imgs/icons/btn_ultimo.gif'  border='0' alt='Ultimo Registro'></a>";
...
$g= "<b>".$numlinhas."</b> coleções encontradas - Página: $page_atual de $numpages &nbsp $lista_combo &nbsp;
