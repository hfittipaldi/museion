...
<table width="100%"  border="0" cellpadding="0" cellspacing="1" >
        <tr>
          <td colspan="2" class="texto"><div align="center"><em>Certifique-se
                inicialmente se a moldura j&aacute;  est&aacute;
...
if($_REQUEST[procurar]<>'')
{
 ...
 $sql="SELECT a.mold_registro from moldura as a where a.mold_registro='".trim($num)."'";
 $db->query($sql);
  $conta=$db->contalinhas();
  if($conta==0)
  { 
     echo"<script>
	 var ok=confirm('Moldura disponível para inclusão.Continua?')
	 if(ok)
	 window.location='cadastro_moldura.php?op=insert&mold_registro=$num';
	 </script>";
 }
  if($conta!=0)
  {
    echo"<script> alert('Moldura:$num já se encontra cadastrado!')</script>";
