...
$db=new conexao();
$db->conecta();
$val=base64_decode($_REQUEST['conserva']);
$op=$_REQUEST['op'];
echo $_SESSION['lnk'];
?>
</div></th>
...
{ 
 if($op=='update')
 {
   $sql="SELECT a.* from estado_conserv as a where a.estado_conserv='$val'";
   $db->query($sql);
   $res=$db->dados();
  }
 if($op=='del')
 {
     $sql="DELETE from estado_conserv where estado_conserv='$val'";
	 $db->query($sql);
	 echo"<script>alert('Exclusão realizada com sucesso')</script>";
	 echo"<script>location.href='conservacao.php'</script>";
	 exit();
  }	 
}
	 ?>
<table width="100%"  border="0" cellpadding="0" cellspacing="4">
        <tr>
          <td colspan="2" class="texto_bold"><div align="center"><br><br><br>Estado de conservação: &nbsp;              
              <input name="estado_conserva" type="text" class="combo_cadastro" id="estado_conserva" value="<? echo $res['descricao'] ?>" size="48">
          </div></td>
        </tr>
        <tr>
          <td colspan="2">
        ...
        <tr>
          <td>
            <div align="left"><? echo "<a href=\"conservacao.php\"><img src='imgs/icons/btn_voltar.gif' width='20' height='20' border='0' alt='Voltar' >"?></div></td>
          <td>&nbsp;</td>
        </tr>
      </table>
      <br>
...
if($_REQUEST['enviar']<>'')
{
  if($_REQUEST[op]=='update')
   {
     $sql="UPDATE estado_conserv set descricao='$_REQUEST[estado_conserva]' 
	 where estado_conserv='$_REQUEST[val]'";
	 $db->query($sql);
  ...
  //   verifica_existencia($_REQUEST[nome]);
     $sql= "INSERT INTO estado_conserv(descricao) values('".trim($_REQUEST[estado_conserva])."')";
	 $db->query($sql);
