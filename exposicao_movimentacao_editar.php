...
if($_REQUEST['enviar']<>'') {
  if($op == 'update') {
	...
	$db->query($sql);
        ...
        if ($cont[total] > 0 )
        {
// REMOVIDO EM 01/07/2010 PORQUE NÃO EXISTE O CAMPO PREMIO NA TABELA	    $sql2="UPDATE movimentacao_exposicao set premio='$_REQUEST[premio]' where movimentacao='$valor' AND exposicao='$id'";
// VERIFICAR SE NÃO ESTÁ CRIANDO PROBLEMAS EM OUTRAS ÁREAS DO DONATO	    $db->query($sql2); 
...
//////////////////////////////////////////////////////////////////
	...
	echo "<script>location.href='exposicao_movimentacao.php?lista=1&instituicaop=".htmlentities(str_replace("\\","",$instituicaop), ENT_QUOTES)."&nomep=".htmlentities(str_replace("\\","",$nomep), ENT_QUOTES)."&".$parametro."=".$valor."';</script>";
...
?>
          <table width="100%" height="20"  border="0" cellpadding="0" cellspacing="0">
             ...
             <tr>
                ...
                <td height="24" class="texto_bold" ><div align="left"> Detalhes da exposição a vincular nº <? echo $row['exposicao']; ?></div></td>
                <td ><? echo "<a href=\"exposicao_movimentacao.php?lista=1&instituicaop=".htmlentities(str_replace("\\","",$instituicaop), ENT_QUOTES)."&nomep=".htmlentities(str_replace("\\","",$nomep), ENT_QUOTES)."&".$parametro."=".$valor."&tipo=".$tipo."\"'><img src='imgs/icons/btn_voltar.gif' border='0' alt='Voltar'>"; ?></td>
