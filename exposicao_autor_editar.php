...
if ($tipo="autor") {
  ...
  if ($op == 'update') {
    ...
    $sql="SELECT * from exposicao where exposicao='$expid'";
    $db->query($sql);
    $row=$db->dados();
	if ($tipo == 'autor') {
	    $sql="SELECT premio from autor_exposicao where autor='$valor' AND exposicao='$expid'";
    	$db->query($sql);
	    $prem=$db->dados();
	}
  }
  ...
  elseif($op=='delexp') {
	...
	$sql= "DELETE from exposicao where exposicao = '$expid'";
	$db->query($sql);
	echo "<script>location.href='exposicao_$tipo.php?lista=".$lista."&".$parametro."=".$valor."';</script>";
   }
  elseif($op=='del') {

	$sql= "DELETE from autor_exposicao where autor_exposicao =".$id;
...
if($_REQUEST['enviar']<>'') {
  if($op == 'update') {
	...
	if ($tipo == 'autor') {
		$sql2="UPDATE autor_exposicao set premio='$_REQUEST[premio]' where autor='$valor' AND exposicao='$expid'";
	    $db->query($sql2); 
	}
	if ($tipo == 'autor') {
...
//////////////////////////////Tabela Log_atualizacao/////////////////////////////
        ...
        $registro=$dbreg->dados(); 
        $obs1="Alteração autor ID={".$valor."}";
        $obs1=$obs1. "Ação={ALteração exposição:".$expid."}";
        $sql="insert into log_atualizacao(operacao,usuario,autor,obra,data,obs)values('A','$_SESSION[susuario]','$valor','0',now(),'$obs1')";
        $db->query($sql);
...
//////////////////////////////Tabela Log_atualizacao/////////////////////////////
        ...
        $registro=$dbreg->dados(); 
        $obs1="Alteração autor ID={".$valor."}";
        $obs1=$obs1. "Ação={Inclusão nova Exposição:".$lastid."}";
        $sql="insert into log_atualizacao(operacao,usuario,autor,obra,data,obs)values('A','$_SESSION[susuario]','$valor','0',now(),'$obs1')";
        $db->query($sql);
...
<table width="100%"  border="0" cellpadding="0" cellspacing="4"><form name="form1" method="post" onSubmit='return valida();' >
        <tr class="texto_bold">
          <td colspan="3"><div align="left" style="color: gray;">
			<? if ($expid <> '') { ?><b>Nº da Exposição: </b><? echo $expid."<br>&nbsp;"; } ?></div>
		  </td>
		  <td width="50%">&nbsp;</td>
        </tr>
        <tr class="texto_bold">
...
<select name="estado" class="combo_cadastro" id="estado" >
  ...
  <? } ?>
        ...
        <tr>
               <td ><? echo "<a href=\"exposicao_autor.php?lista=".$lista."&".$parametro."=".$valor."&tipo=".$tipo."\"'><img src='imgs/icons/btn_voltar.gif' border='0' alt='Voltar'>"; ?></td>
		...
		<input type="hidden" name="expid" value="<? echo $expid; ?>">
