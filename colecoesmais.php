...
<html>
<head>
<title>Coleções Mais Consultadas</title>
...
var i = qual.value;
document.location=('colecoesmais.php?page='+ i+ '&dt_ini=<? echo $_REQUEST[dt_ini]; ?>&dt_fim=<? echo $_REQUEST[dt_fim]; ?>');
 }
}

function posiciona(valor) {
var i = valor;
document.location=('colecoesmais.php?page='+ i+ '&titulo=<? echo $_REQUEST[titulo] ?>&vinculo=<? echo $_REQUEST[vinculo] ?>&funcao=<? echo $_REQUEST[funcao] ?>&cor=<? echo $_REQUEST[cor] ?>');
}
...
?>
      ...
      <?
	  /*function total_obras() // obtem total de obras naquela colecao especifica
	  {
	    global $db;
		*/
	  /////Paginando
	  ...
	  $registroinicial=$page* $pagesize;
		...
		if ($_REQUEST['dt_ini']=='' && $_REQUEST['dt_fim']=='') {
		  /*$sql="SELECT count(distinct a.colecao) as total from log_pesquisa as a,colecao as b where (a.colecao=b.colecao) and
		        a.colecao<>0 ";
		  $sql2="SELECT a.log_pesquisa,a.colecao,b.nome,sum(a.quantidade) as total FROM (log_pesquisa AS a, colecao AS b) 
			         WHERE (a.colecao=b.colecao) group by a.colecao order by sum(a.quantidade) desc LIMIT $registroinicial,$pagesize"; */
			$sql="SELECT count(distinct colecao) as total from colecao where colecao <> 0";
		  $sql2="SELECT *, quantidade as total FROM colecao group by colecao order by quantidade desc LIMIT $registroinicial,$pagesize";
		}
		else {
		  $dti=seta_data($_REQUEST[dt_ini]);
		  $dtii=seta_data($_REQUEST[dt_fim]);
		  
/*		  $sql="SELECT count(distinct a.colecao) as total from log_pesquisa as a,colecao as b where (a.colecao=b.colecao) 
		  and a.colecao<>0 and a.data_hora>='$dti' and a.data_hora < DATE_ADD('$dtii',INTERVAL 1 DAY)";
		  $sql2="SELECT a.colecao,a.log_pesquisa,b.nome,sum(a.quantidade) as total FROM (log_pesquisa AS a, colecao as b) 
			        WHERE (a.colecao=b.colecao) and a.data_hora >='$dti' and a.data_hora < DATE_ADD('$dtii',INTERVAL 1 DAY) 
		                group by b.colecao order by sum(a.quantidade) desc LIMIT $registroinicial,$pagesize"; */
			$sql="SELECT count(distinct colecao) as total from colecao where colecao <> 0";
		  $sql2="SELECT *, quantidade as total FROM colecao group by colecao order by quantidade desc LIMIT $registroinicial,$pagesize";
		}
		  $db->query($sql);
	   ...
	   ?>
  <tr>
    <td valign="top" class="texto_bold"><form name="form2" method="get" action="colecoesmais.php" onSubmit="return valida()">
      <div align="right"><? if ($_REQUEST[pagesize] >= 999) { /*echo "Coleções mais consultadas";*/ } ?><!--Per&iacute;odo de busca: de
		<? if ($_REQUEST[pagesize] < 999) { ?>
  ...
  <tr>
    <td valign="top"><form name="form1" method="post" action="">
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
        ...
        <tr bgcolor="#ddddd5">
          <td width="11%" height="24" bgcolor="#ddddd5" class="texto_bold"><div align="left"> &nbsp;Cole&ccedil;&otilde;es</div></td>
	  ...
	  ?>
        <tr id="cor_fundo<? echo $row['colecao'] ?>" class="texto">
          <td colspan="2" align="left"><b><? echo $row['nome'] ?></b>
	 ...
	 if ($_REQUEST[pagesize] < 999) echo "<a href=\"colecoes_obra.php?col=".$row['colecao']."\"> 
	 <img src='imgs/icons/ic_adicionar.gif' width='20' height='20' border='0' alt='Listar obras da coleçao' 
     ...
     <? } ?>
        ...
        <tr class="texto">
          <td colspan="4" class="noprint"><? if ($_REQUEST[pagesize] < 999) echo "<a target='_blank' href=\"colecoesmais.php?pagesize=999999&page=1&dt_ini=".$_REQUEST[dt_ini]."&dt_fim=".$_REQUEST[dt_fim]."\"><img src='imgs/icons/ic_salvar_impressao.gif'  border='0'  alt='Versão para impressão'></a>" ?></td>
        </tr>
        <tr>
...
$a="<a href=\"colecoesmais.php?page=".$first."&dt_ini=".$_REQUEST[dt_ini]."&dt_fim=".$_REQUEST[dt_fim]."\"><img src='imgs/icons/btn_inicio.gif'  border='0'  alt='Registro Inicial' ></a>";

$b="<a href=\"colecoesmais.php?page=".$menos."&dt_ini=".$_REQUEST[dt_ini]."&dt_fim=".$_REQUEST[dt_fim]."\"><img src='imgs/icons/btn_anterior.gif'  border='0' alt='Registro Anterior' ></a>";

$c="<a href=\"colecoesmais.php?page=".$mais."&dt_ini=".$_REQUEST[dt_ini]."&dt_fim=".$_REQUEST[dt_fim]."\"><img src='imgs/icons/btn_proximo.gif'  border='0' alt='Proximo Registro' ></a> ";

$d="<a href=\"colecoesmais.php?page=".$last."&dt_ini=".$_REQUEST[dt_ini]."&dt_fim=".$_REQUEST[dt_fim]."\"><img src='imgs/icons/btn_ultimo.gif'  border='0' alt='Ultimo Registro' ></a>";
$combo="";
